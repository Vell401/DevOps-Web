...
function ProjectGlyph({ keyText, active }: { keyText: string; active: boolean }) {
  return (
    <span
      className={cn(
        'inline-flex h-5 min-w-[26px] items-center justify-center rounded-sm px-1 font-mono text-[10px] font-medium transition',
        active
          ? 'bg-blurple/20 text-[#A8B0F8]'
          : 'bg-surface-deep text-ink-subtle group-hover:bg-surface/60',
      )}
    >
      {keyText.slice(0, 4)}
    </span>
