...
- **Список пользователей** — с количеством их проектов, задач и комментариев.
  ...
  - Иконка корзины удаляет пользователя (каскадно — его проекты и
    пространства Docs вместе с их содержимым, задачи, комментарии).
- **Recent signups** — последние пять регистраций.

### Все проекты в системе (`/admin/projects`)

Третий пункт админки — список всех проектов независимо от владельца,
открытых и закрытых. Поиск по названию/ключу/владельцу, фильтр
«All / Open / Closed», в таблице — ключ проекта, название (ссылка на
проект), владелец, число участников, прогресс по задачам (готово/всего),
статус и дата создания. Пагинация — кнопкой «Load more».

### Страница системных метрик (`/admin/metrics`)

Вторая вкладка админки — живой per-service дашборд (обновление каждые 10 с):

- **Карточки сервисов** со статусом (индикатор: зелёный/красный/серый — «нет данных»), аптаймом и размерами: backend
...
`DocSpace` и всё вложенное в неё — `DocSpaceMember`, `DocPage`, `DocImage`,
`DocPageRevision`) и связующие many-to-many (`_TaskLabels`, `_TaskAssignees`):
...
с их задачами, лейблами, комментариями и активностью), а также пространства
Docs, которыми он владеет (вместе со всеми страницами, изображениями и
историей версий в них), его комментарии, записи активности и refresh-токены.
Из задач и проектов, где пользователь был лишь исполнителем или участником,
удаляются только связи — сами задачи и проекты сохраняются; то же верно для
пространств Docs, в которые он был лишь приглашён.
