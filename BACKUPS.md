...
Важно различать, **что именно** меняется — от этого зависит, нужны ли действия
с systemd.

- **Меняется только скрипт** (`tracker-backup.sh` — расписание GFS внутри,
  логика бэкапа): systemd трогать **не нужно**. Таймер каждый раз исполняет файл
  по пути `/opt/tracker/deploy/tracker-backup.sh`; достаточно положить новую
  версию (её и так раскатывает CI на деплое, либо скопировать вручную). Новый
  код применится со следующего прогона — проверить сразу: `sudo systemctl start
  tracker-backup.service`.

- **Меняется unit** (`.timer` — расписание `OnCalendar`; `.service` — пути,
  `Environment`): скопировать новый файл поверх старого и перечитать конфигурацию.
  **Удалять/переустанавливать службу не нужно**, симлинк от `enable` сохраняется:

  ```bash
  sudo cp /opt/tracker/deploy/systemd/tracker-backup.timer /etc/systemd/system/
  sudo systemctl daemon-reload
  sudo systemctl restart tracker-backup.timer   # для .timer — перечитать расписание
  # (для .service отдельный restart не нужен — применится на следующем запуске)
  ```

- **Локальная правка без редактирования shipped-файла** (добавить
  `BACKUP_CHECK_READ_DATA=1`, `BACKUP_PING_URL=…` и т.п.) — предпочтительно
  через drop-in, чтобы CI-обновления unit'а её не затирали:

  ```bash
  sudo systemctl edit tracker-backup.service   # создаст override.conf, daemon-reload автоматом
  # в редакторе:
  #   [Service]
  #   Environment=BACKUP_CHECK_READ_DATA=1
  ```

Проверить итоговую конфигурацию (с учётом drop-in'ов) и расписание:

```bash
systemctl cat tracker-backup.service          # эффективный unit + override'ы
systemctl list-timers tracker-backup.timer    # следующий запуск
```
...
скрипт пишет `/opt/tracker/backups/status.json` (без секретов), а бэкенд монтирует
этот файл **read-only** (`BACKUP_STATUS_FILE`) и показывает на `/admin/metrics`.

Содержимое `status.json` и что видно в админке:
- **Статус-карточка** — общий результат (зелёная «Healthy» / жёлтая «Stale», если
  старше ~7 ч / красная «Failed»), время последнего прогона, число снапшотов,
  размер репозитория, самый старый снапшот (глубина истории) и политика хранения
  (`7d / 4w / 3m`), результат последнего `check`.
- **«Last 3 days»** — по каждому из трёх последних дней галочки, есть ли успешный
  снапшот БД и файлов (день успешен, если есть оба).
- **«Recent snapshots»** — лог последних ~12 снапшотов (тег `db`/`files`, время,
  короткий id) — это и есть «журнал» успешных прогонов.

Журнальные поля (`recent`, `oldest`) строит `jq` (ставится в RUNNING-VM §1.1).
Если `jq` на хосте нет — карточка работает, но без таймлайна и лога (только
базовые цифры). Карточка дополнительно служит визуальным dead-man-сигналом.
